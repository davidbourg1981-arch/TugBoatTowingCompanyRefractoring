function updateCompetitors(delta = 1) {
  ...
  while (activeCompetitors.length < targetCount) {
    const newComp = createCompetitor(activeCompetitors.length);
    newComp.waitTimer = 1; // Start immediately
    activeCompetitors.push(newComp);
  }
...
function moveCompetitorToward(comp, targetX, targetY, delta = 1) {
  ...
  for (const other of activeCompetitors) {
